...
import com.example.clothingstoreadmin.databinding.FragmentShippingOrderBinding
...
class ShippingOrderFragment : Fragment() {
    private lateinit var _binding: FragmentShippingOrderBinding
    ...
    override fun onCreateView(
        ...
        _binding = FragmentShippingOrderBinding.inflate(inflater,container,false)
        initView()
        handleClick()
        return binding.root
    }
    ...
    fun loadNextPage(){
        ...
        orderService.getNextPage(ProgressOrder.Shipping.name){
            if(it.isEmpty()){
                isLastPage.set(true)
                isLoading.set(true)
    ...
    private fun getFirsData(){
        ...
        orderService.getOrderByStatus(ProgressOrder.Shipping.name) {
            if(it.isEmpty()){
                isLoading.set(true)
                isLastPage.set(true)
